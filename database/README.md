...
This folder is the **only definition of the schema**. There is no migration tool: each file in `migrations/` is a plain SQL script that a person pastes into the Supabase **SQL Editor** and runs once, in numeric order (`001` to `025` at the time of writing).
