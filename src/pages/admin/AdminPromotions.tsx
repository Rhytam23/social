...
export function AdminPromotions() {
  ...
  const fieldClass = 'w-full bg-[var(--bg-surface-secondary)] border border-[var(--border-theme)] rounded-lg p-2.5 text-xs text-[var(--text-primary)] focus:outline-none focus:border-[var(--accent-blue)] placeholder:text-[var(--text-secondary)]'
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="bg-[var(--bg-surface)] border border-[var(--border-theme)] rounded-xl p-6 shadow-sm">
        <div className="flex items-center justify-between mb-4 pb-3 border-b border-[var(--border-theme)]">
          <div>
            <h2 className="text-[var(--text-primary)] font-bold text-base flex items-center gap-2">
              <Icon name="campaign" size={20} className="text-[var(--accent-blue)]" />
      ...
      {/* Coupons */}
      <section>
        <h2 className="text-[var(--text-primary)] font-bold text-sm mb-3">Active Coupon Codes</h2>
        <div className="bg-[var(--bg-surface)] border border-[var(--border-theme)] rounded-xl overflow-x-auto">
          <table className="w-full text-xs min-w-[720px]">
            ...
            <tbody className="divide-y divide-[var(--border-theme)]">
              {coupons.map((c) => (
                <tr key={c.id} className="hover:bg-[var(--bg-surface-secondary)]">
                  <td className="p-3">
                    <span className="font-mono text-[var(--accent-blue)] font-bold bg-[var(--accent-blue)]/10 border border-[var(--accent-blue)]/20 px-2 py-0.5 rounded-md">
                      {c.code}
                    </span>
                  </td>
                  <td className="p-3 text-[var(--text-primary)]">{c.description}</td>
                  <td className="p-3 text-center font-mono text-[var(--text-primary)] font-bold">
                    {c.type === 'percent' ? `${c.value}%` : `$${c.value}`}
                  </td>
                  <td className="p-3">
                    <div className="flex items-center gap-2">
                      <div className="w-16 h-1.5 bg-[var(--bg-surface-secondary)] rounded-full overflow-hidden">
                        <div className="h-full bg-[var(--accent-blue)]" style={{ width: `${Math.min(100, (c.uses / c.maxUses) * 100)}%` }} />
                      </div>
                  ...
                  <td className="p-3 font-mono text-[var(--text-secondary)]">{c.expires}</td>
