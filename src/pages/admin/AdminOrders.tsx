  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-4">
        <div className="flex gap-1 bg-(--bg-surface-secondary) border border-(--border-theme) rounded-lg p-1 overflow-x-auto scrollbar-none">
          {STATUSES.map((s) => (
            <button
              key={s}
              onClick={() => setStatus(s)}
              className={`px-3 py-1.5 rounded-md font-sans text-xs font-semibold whitespace-nowrap transition-colors cursor-pointer ${
                status === s ? 'bg-(--accent-blue) text-white' : 'text-(--text-secondary) hover:text-(--text-primary)'
              }`}
            >
              {s}
            </button>
          ...
          <table className="w-full text-xs min-w-[850px]">
            ...
            <tbody className="divide-y divide-(--border-theme)">
              {filtered.map((o) => {
                ...
                return (
                  <tr key={o.id} className="hover:bg-(--bg-surface-secondary)">
                    <td className="p-3 font-mono text-(--accent-blue) font-bold">#{o.id}</td>
                    <td className="p-3">
                      <div className="text-(--text-primary) font-semibold">{custName}</div>
                      <div className="font-mono text-[10px] text-(--text-secondary)">{custEmail}</div>
                    </td>
                    <td className="p-3 text-center font-mono text-(--text-secondary)">{o.items?.length || 1}</td>
                    <td className="p-3 text-right font-mono text-(--text-primary) font-bold">${o.total?.toFixed(2)}</td>
                    ...
                    <td className="p-3 font-mono text-(--text-secondary)">{o.date}</td>
                    <td className="p-3 text-right">
                      <select
                        value={o.status}
                        onChange={(e) => updateOrderStatus(o.id, e.target.value as OrderStatus)}
                        className="bg-(--bg-surface-secondary) border border-(--border-theme) text-(--text-primary) font-mono text-[11px] rounded-lg px-2 py-1 focus:outline-none focus:border-(--accent-blue)"
                      >
