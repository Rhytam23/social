    ...
    <div>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-4">
        <div className="relative flex items-center bg-[var(--bg-surface-secondary)] border border-[var(--border-theme)] rounded-lg px-3 py-2 flex-1 min-w-[200px]">
          ...
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search by product, SKU, supplier..." className="w-full bg-transparent text-xs text-[var(--text-primary)] focus:outline-none placeholder:text-[var(--text-secondary)]" />
        </div>
        <label className="flex items-center gap-2 text-xs font-sans text-[var(--text-primary)] cursor-pointer bg-[var(--bg-surface-secondary)] border border-[var(--border-theme)] rounded-lg px-3 py-2 select-none">
          <input type="checkbox" checked={lowOnly} onChange={(e) => setLowOnly(e.target.checked)} className="w-4 h-4 rounded bg-[var(--bg-surface)] border-[var(--border-theme)] text-[var(--accent-blue)]" /> Low Stock Only
        </label>
      </div>
          ...
          <table className="w-full text-xs min-w-[860px]">
            ...
            <tbody className="divide-y divide-[var(--border-theme)]">
              {filtered.map((r) => {
                ...
                return (
                  <tr key={r.product.id} className="hover:bg-[var(--bg-surface-secondary)]">
                    <td className="p-3">
                      <div className="flex items-center gap-3 min-w-0">
                        <img src={r.product.image} alt="" className="w-9 h-9 object-cover rounded-md bg-[var(--bg-surface-secondary)] shrink-0" />
                        <span className="text-[var(--text-primary)] font-semibold truncate max-w-[200px]">{r.product.name}</span>
                      </div>
                    </td>
                    <td className="p-3 font-mono text-[var(--text-secondary)]">{r.product.sku}</td>
                    <td className="p-3 text-right font-mono text-[var(--text-primary)]">{r.product.stockCount ?? 0}</td>
                    <td className="p-3 text-right font-mono text-[var(--color-stock-yellow-val)]">{r.reserved}</td>
                    <td className="p-3 text-right font-mono text-[var(--color-stock-green)] font-bold">{available}</td>
                    <td className="p-3 text-center"><Pill status={r.product.stockStatus} /></td>
                    <td className="p-3 text-[var(--text-primary)]">{r.supplier}</td>
                    <td className="p-3 font-mono text-[var(--text-secondary)]">{r.restockEta}</td>
