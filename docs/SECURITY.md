...
- **Message requests.** In a direct chat, a person the other side has not yet answered can send 3 messages; the 4th is refused by the database until the other person sends anything (`message_request_limit`, migration `026`). The state is kept on the conversation, so deleting messages, disappearing messages or leaving and being re-added do not reset it, and members cannot edit it. Platform admins are not limited. **Limits:** it applies to direct chats; groups are covered by invitations (next item); and it limits volume, it does not stop what those 3 messages say (report and block still apply).
- **Group invitations.** Nobody is put into a group by someone they do not know (migration `027`). A person the adder knows (a direct chat where the adder was answered, or a shared community) is added at once; anyone else gets an invitation, kept 14 days (at most 30 waiting for one person, 30 sent an hour per person), and joins only when they accept; the invitation stops working if the sender is no longer an owner or admin. Clients cannot insert group members at all any more; the only ways in are `add_group_member` and `respond_group_invite`. A person who blocked the adder is skipped and the adder is not told. **Limits:** an invitation is still a message from a stranger (their name and the group name); the key for the group reaches a new member only when an admin's device is online to share it.
...
- **People are found by the start of their username only** (`/api/users?username=`, 3 characters or more, at most 8 results, 40 a minute and 1500 a day per account); email and phone are never returned and are hidden by column privileges (`011`, `016`). Since `028` **strangers' profiles cannot be read through the database API at all**: a profile is visible to its owner, platform admins, people in a conversation or community with that person (including people who left a chat with you) and people you blocked, and search is a server-only function, so its limits cannot be skipped. **Limit:** anyone can still find a person whose exact username they already know only by searching for it (that is the feature), and they can try many usernames within the 1500-a-day cap.
