...
- `lib/messaging/attachments.ts`: encrypts a file client-side, uploads ciphertext through a signed address from `/api/uploads/sign`, returns an envelope; download does the reverse via `supabase.storage.download()` (RLS-protected) + client-side decrypt.
