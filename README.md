...
**Platform features** (need migrations 011 to 022 and 025; see [Roadmap](docs/ROADMAP.md#what-is-verified) for what has and has not been tested)
...
2. In the Supabase **SQL Editor**, run every file in `database/migrations/` **once, in strict numeric order** (`001` to `022` and `025`). Each needs the ones before it. `017` and `018` carry security fixes; `020` is a performance improvement the app works without.
...
database/     SQL migrations 001-022 and 025: the only definition of the schema
