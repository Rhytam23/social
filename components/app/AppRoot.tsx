...
/**
 * The signed-in application and its boot sequence. Loaded on demand by HomeGate, only for visitors who
 * appear to have a session, so the public landing page never downloads any of this.
 * `landing` is what a visitor without a valid session sees.
 */
export function AppRoot({ landing }: { landing: React.ReactNode }) {
  ...
  if (authLoading) {
    return (
      <div role="status" className="h-screen w-screen bg-[var(--canvas-bg)] flex flex-col items-center justify-center font-sans">
  ...
  // Not signed in: the public landing page. Its buttons go to the /login and /signup routes.
