...
export async function GET(request: NextRequest) {
  const limited = await limitByIp(request, 'grp-get', { limit: 120, windowMs: 60 * 1000 });
  ...
  if (!isUuid(groupId)) {
    return NextResponse.json({ error: 'A valid groupId parameter is required.' }, { status: 400 });
  }

  // IDOR protection: the caller must be an active member of this group.
  ...
  return NextResponse.json({ ...groupData, members: members || [] });
}
