 ...
 *   (Served by search_profiles_by_prefix, callable only with the service key: profiles of strangers are not
 *   readable through the database API at all, migration 028.)
 ...
 *   email, phone number and bio are never searched, and email/phone are never returned. Platform admins and suspended
 *   accounts are never returned. Rate limited per person,
...
async function lookupByUsername(supabase: Awaited<ReturnType<typeof createServerClient>>, myId: string, raw: string) {
  ...
  // Finding someone new goes through a database function only the server can call (migration 028), so these limits
  // cannot be skipped by asking the database API directly. The key is required: without it there is no search.
  if (!process.env.SUPABASE_SERVICE_ROLE_KEY) {
    return NextResponse.json({ error: 'Search is not available right now.' }, { status: 503 });
  }
  const server = createAdminClient() as unknown as {
    rpc: (fn: string, args: Record<string, unknown>) => Promise<{ data: unknown; error: { message?: string } | null }>;
  };
  const { data, error } = await server.rpc('search_profiles_by_prefix', { p_caller: myId, p_prefix: parsed.value, p_limit: MAX_MATCHES });
