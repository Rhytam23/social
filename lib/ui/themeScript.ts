 ...
 */
export const THEME_INIT_SCRIPT = `try{if(/(?:^|;\\s*)sb-[^=]*-auth-token/.test(document.cookie))document.documentElement.setAttribute('data-session','1')}catch(e){}try{var t=localStorage.getItem('${THEME_STORAGE_KEY}');if(t!=='light'&&t!=='dark')t='system';document.documentElement.setAttribute('data-theme',t)}catch(e){document.documentElement.setAttribute('data-theme','system')}`;
